...
class Settings:
    leaf_detector_path: Path = Path(
        os.getenv("LEAF_DETECTOR_PATH", MODELS_DIR / "leaf_detector_yolo26n.pt")
    )
    disease_classifier_path: Path = Path(
        os.getenv(
            "DISEASE_CLASSIFIER_PATH",
            MODELS_DIR / "disease_classifier_efficientnet_v2_s.pt",
        )
    )
    ...
    crop_padding: float = float(os.getenv("LEAF_CROP_PADDING", "0.08"))
